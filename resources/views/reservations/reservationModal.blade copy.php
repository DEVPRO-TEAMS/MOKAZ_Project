        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="step-content active" id="step1">
                    <div class="row g-3">
                        ...
                        {{-- <div class="col-12">
                            ...
                            <div class="d-flex gap-3">
                                @if(($tarifJour > 0 && $tarifHeure == 0) || ($tarifJour > $tarifHeure))
                                    <div class="card p-3 flex-fill text-center type-sejour-card" data-type="jour"
                                        style="cursor: pointer;">
                                        <input type="radio" checked name="sejour" id="sejour_jour" class="btn-check"
                                            autocomplete="off" value="jour">
                                        <label class="btn btn-outline-danger w-100" for="sejour_jour">
                                            <i class="fas fa-calendar-day me-2"></i>Par Jour
                                        </label>
                                    </div>
                                @endif
                                @if(($tarifHeure > 0 && $tarifJour == 0) || ($tarifHeure > $tarifJour))
                                    <div class="card p-3 flex-fill text-center type-sejour-card" data-type="heure"
                                        style="cursor: pointer;">
                                        <input type="radio" checked name="sejour" id="sejour_heure" class="btn-check"
                                            autocomplete="off" value="heure">
                        ...
                        @if($tarifHeureCount > 0 || $tarifJourCount > 0)
                            ...
                            <div class="col-12">
                                ...
                                <div class="d-flex gap-3 flex-wrap">
                                    ...
                                    <input type="hidden" name="tarifJour" id="tarifJour" value="{{ $tarifByDay->price }}">
                                    @endif
                        ...
                        <div id="bloc-jour" class="row g-3 {{ $checkedType === 'jour' ? '' : 'd-none'}}">
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control form-control-lg" name="start_time"
                                    id="start_time">
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control form-control-lg" name="end_time"
                                    id="end_time">
                            </div>
                        </div>
                        
                        <!-- Bloc: Nombre d'heures si "heure" est sélectionné -->
                        <div id="bloc-heure" class="row g-3 col-md-12 {{ $checkedType === 'heure' ? '' : 'd-none'}}">
                            <div class="col-md-6 d-flex flex-column justify-content-center align-items-start">
                                @foreach($appart->tarifications->where('etat','!=', 'inactif')->where('sejour', 'Heure') as $tarif)
                                    <div class="form-check mb-3">
                                        <input class="form-check-input" type="radio" value="{{ $tarif->price }}" name="tarif_by_sejour" id="tarifBySejour{{ $tarif->uuid }}" required>
                                        <label class="form-check-label" for="tarifBySejour{{ $tarif->uuid }}">
                                            <strong>{{ $tarif->price }} FCFA / {{ $tarif->nbr_of_sejour }} {{ $tarif->sejour }}{{ $tarif->nbr_of_sejour > 1 ? 's' : '' }}</strong>
                                        </label>
                                        <input type="hidden" name="tarifBySejour" id="tarifBySejour" value="{{ $tarif->price }}">
                                        <input type="hidden" name="timeBySejour" id="timeBySejour" value="{{ $tarif->nbr_of_sejour }}">
                                    </div>
                                @endforeach
                                
                                <div class="form-check mb-3">
                                    <input class="form-check-input" type="radio" value="autreTarif" name="tarif_by_sejour" id="tarifAutre" required>
                                    <label class="form-check-label" for="tarifAutre">
                                        <strong> Autre tarif</strong>
                                    </label>
                                </div>
                            </div>
                            <div id="sejourAutreNumber" class="col-md-6 d-none">
                                <label for="nbr_of_sejour" class="form-label">
                                    <i class="fas fa-hourglass-half me-2"></i>Nombre d'heures souhaitées *
                                </label>
                                <input type="number" min="1" class="form-control form-control-lg"
                                    name="nbr_of_sejour" id="nbr_of_sejour" placeholder="Ex: 3 heures">
                            </div>

                            <div id="sejourAutreTime" class="row g-3">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="time" class="form-control form-control-lg" name="time_start"
                                        id="time_start" readonly>
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="priceHoursPreview" class="card bg-danger text-white mt-4 d-none">
                        <div class="card-body">
                            <h5 class="card-title"><i class="fas fa-calculator me-2"></i>Aperçu des prix</h5>
                            <div class="d-flex justify-content-between mb-2">
                                <span id="UnitpriceHourTitle"></span>
                                <span id="UnitpriceHour">0 XOF</span>
                            </div>
                            <div class="d-flex justify-content-between mb-2">
                                <span id="HoursCountTitle"></span>
                                <span id="HoursCount">0</span>
                            </div>
                            <div class="d-flex justify-content-between border-top pt-2 fw-bold">
                                <span>Total:</span>
                                <span id="totalAmountHour">0 XOF</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success" id="payBtn" onclick="processPayment()"
                    style="display: none;">
...
<script>
    ...
    const tarifHeureSort = @json($tarifHeureSort);
    const appart_uuid = @json($appart->uuid);
    let currentStep = 1;
    let reservationData = {};
    ...
    function setupEventListeners() {
        ...
        document.getElementById('time_start').addEventListener('input', function() {
    ...
    function calculatePrice() {
        const tarifRadios = document.querySelectorAll('input[name="tarif_by_sejour"]');
        const nbrOfSejourInput = document.getElementById('nbr_of_sejour');
        const tarifBySejour = document.getElementById('tarifBySejour');
        const timeBySejour = document.getElementById('timeBySejour');
        const startDate = document.getElementById('start_date');
        const startTime = document.getElementById('time_start');
        // if (!startDate?.value || !startTime.value) return;

        // const start = new Date(startDate.value + ' ' + startTime.value);
        const nbrOfSejour = parseInt(nbrOfSejourInput.value);

        tarifRadios.forEach(radio => {
            radio.addEventListener('change', function () {
                if (this.value == 'autreTarif') {
                    if (nbrOfSejour > 0) {
                        const total = nbrOfSejour * tarifHeureSort;
                        const paymentAmount = total * 0.1; // 10% d'acompte
                        const Unitprice = document.getElementById('UnitpriceHour').textContent = tarifHeureSort + ' XOF';
                        document.getElementById('UnitpriceHourTitle').textContent = 'Tarif de base';
                        document.getElementById('HoursCountTitle').textContent = 'Nombre d\'heures';
                        document.getElementById('HoursCount').textContent = nbrOfSejour;
                        document.getElementById('totalAmountHour').textContent = total.toLocaleString('fr-FR') + ' XOF';
                        document.getElementById('priceHoursPreview').classList.remove('d-none');

                        // Stocker les données
                        reservationData.days = nbrOfSejour;
                        reservationData.totalPrice = total;
                        reservationData.paymentAmount = paymentAmount;
                        reservationData.unit_price = tarifHeureSort;
                    }
                } else {
                    const total = parseFloat(tarifBySejour.value);
                    const paymentAmount = total * 0.1; // 10% d'acompte
                    const Unitprice = document.getElementById('UnitpriceHour').textContent = total + ' XOF';
                    document.getElementById('UnitpriceHourTitle').textContent = 'Tarif de base';
                    document.getElementById('HoursCountTitle').textContent = 'Nombre d\'heures';
                    document.getElementById('HoursCount').textContent = parseInt(timeBySejour.value);
                    document.getElementById('totalAmountHour').textContent = total.toLocaleString('fr-FR') + ' XOF';
                    document.getElementById('priceHoursPreview').classList.remove('d-none');

                    // Stocker les données
                    reservationData.days = parseInt(timeBySejour.value);
                    reservationData.totalPrice = total;
                    reservationData.paymentAmount = paymentAmount;
                    reservationData.unit_price = total;
                }
            });
        });
    }
    ...
    function validateCurrentStep() {
        const policiesChecked = document.querySelectorAll(
            '#policyPrivacy:checked, #policyRefund:checked, #policyTerms:checked').length === 3;
        ...
        if (currentStep === 1) {
            // Validation étape 1
            const requiredFields = ['nom', 'prenoms', 'email', 'phone', 'start_date', 'time_start', 'nbr_of_sejour'];
            ...
            reservationData = {
                ...
                start_time: document.getElementById('start_date').value,
                end_time: document.getElementById('end_time').value,
                notes: document.getElementById('notes').value.trim(),
                // Ajouter l'ID de la chambre (à définir selon votre logique)
                appart_uuid:  appart_uuid// Remplacez par l'ID réel de la chambre
            };
    ...
    function downloadReceipt() {
        ...
        doc.text(`Période : ${formatDate(reservationData.start_time)} - ${formatDate(reservationData.end_time)}`, 20,
            90);
...
<script>
    ...
    // Variables globales
    const DAILY_RATE = @json($tarifByDay->price);
    ...
    function calculatePrice() {
        ...
        if (days > 0) {
            ...
            const paymentAmount = total * 0.1; // 10% d'acompte
            const Unitprice = document.getElementById('Unitprice').textContent = DAILY_RATE + ' XOF';
